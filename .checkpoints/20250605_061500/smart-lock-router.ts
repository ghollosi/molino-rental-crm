...
export const smartLockRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingLock) {
        throw new TRPCError({
          code: 'CONFLICT',
          message: 'Smart lock with this external ID already exists'
        })
      }

      // Verify lock exists in the platform system
      try {
        let deviceData: any
        
        if (input.platform === 'TTLOCK') {
          deviceData = await ttlockService.getLock(input.externalId)
          if (!deviceData) {
            throw new TRPCError({
              code: 'NOT_FOUND',
          ...
          // For other platforms, use universal factory
          const platform = SmartLockFactory.getPlatform(input.platform as LockPlatform)
          deviceData = await platform.getDevice(input.externalId)
          if (!deviceData) {
            throw new TRPCError({
              code: 'NOT_FOUND',
              message: `${input.platform} device not found`
            })
          }
        }
        ...
        const smartLock = await ctx.db.smartLock.create({
          data: {
            ...
            platform: input.platform as LockPlatform,
            ...
            floor: input.floor,
            lockType: deviceData.lockType || 'UNKNOWN',
            batteryLevel: deviceData.batteryLevel,
            lockStatus: deviceData.lockStatus as any || 'UNKNOWN',
  ...
  syncStatus: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        if (smartLock.platform === 'TTLOCK') {
          deviceData = await ttlockService.getLock(smartLock.externalId)
          if (!deviceData) {
            throw new TRPCError({
              code: 'NOT_FOUND',
              message: 'TTLock device not found'
          ...
          deviceData = await platform.getDevice(smartLock.externalId)
          if (!deviceData) {
            throw new TRPCError({
              code: 'NOT_FOUND',
              message: `${smartLock.platform} device not found`
            })
          }
        }
        ...
        const updatedLock = await ctx.db.smartLock.update({
          ...
          data: {
            batteryLevel: deviceData.batteryLevel,
            lockStatus: deviceData.lockStatus as any,
            signalStrength: deviceData.signalStrength,
            isOnline: deviceData.isOnline,
            lastHeartbeat: deviceData.lastHeartbeat || new Date()
          }
        })
  ...
  createAccessCode: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        // Create passcode in the platform system
        let externalPasscodeId: string
        
        if (smartLock.platform === 'TTLOCK') {
          const ttlockPasscode = await ttlockService.createPasscode({
            lockId: smartLock.externalId,
            passcode,
          ...
          externalPasscodeId = ttlockPasscode.passcodeId
        } else {
          // For other platforms, use universal factory
          const platform = SmartLockFactory.getPlatform(smartLock.platform)
          const platformPasscode = await platform.createAccessCode(smartLock.externalId, {
            passcode,
            startDate: input.startDate,
            endDate: input.endDate,
          ...
          externalPasscodeId = platformPasscode.id
        }
        ...
        const accessCode = await ctx.db.accessCode.create({
          data: {
            ...
            ttlockPasscodeId: externalPasscodeId,
            code: encryptedCode,
            codeType: input.codeType,
  ...
  deleteAccessCode: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        // Delete from platform system
        if (accessCode.ttlockPasscodeId) {
          if (accessCode.smartLock.platform === 'TTLOCK') {
            await ttlockService.deletePasscode(
              accessCode.smartLock.externalId,
              accessCode.ttlockPasscodeId
            )
          } else {
            // For other platforms, use universal factory
            const platform = SmartLockFactory.getPlatform(accessCode.smartLock.platform)
            await platform.deleteAccessCode(accessCode.smartLock.externalId, accessCode.ttlockPasscodeId)
          }
        }
      ...
      try {
        ...
        if (smartLock.platform === 'TTLOCK') {
          if (input.action === 'unlock') {
            ...
            success = await ttlockService.lockRemotely(smartLock.externalId)
          }
        } else {
          // For other platforms, use universal factory
          const platform = SmartLockFactory.getPlatform(smartLock.platform)
          if (input.action === 'unlock') {
            success = await platform.unlock(smartLock.externalId, input.reason)
          } else {
            success = await platform.lock(smartLock.externalId)
          }
        }
